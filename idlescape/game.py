...
class Game:
    ...
    def _get_current_activity(self, character_name, session: sqlalchemy.orm.Session) -> Optional[CharacterActivity]:
        """
        ...
        Args:
            ...
            session: SQLAlchemy session.
        Returns:
            CharacterActivity ORM object or None if not found.
        """
        character = self._get_character_by_name(character_name, session)
        current_activity = (
            session.query(CharacterActivity).filter_by(character_id=character.character_id, ended_at=None).one_or_none()
